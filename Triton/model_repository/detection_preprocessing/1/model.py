...
class TritonPythonModel:
    def initialize(self, args: T.Dict[str, T.Any]) -> None:
        ...
        shape_config = pb_utils.get_output_config_by_name(
            model_config, "detection_preprocessing_shape"
        )

        self.output_dtype = pb_utils.triton_string_to_numpy(
            output_config["data_type"]
        )
        self.shape_dtype = pb_utils.triton_string_to_numpy(
            shape_config["data_type"]
        )
    ...
    def execute(self, requests):
        ...
        for request in requests:
            ...
            output_tensor = pb_utils.Tensor(
                "detection_preprocessing_output", image.astype(self.output_dtype)
            )
            shape_tensor = pb_utils.Tensor(
                "detection_preprocessing_shape", shape.astype(self.shape_dtype)
            )

            response = pb_utils.InferenceResponse(
                output_tensors=[output_tensor, shape_tensor]
            )
