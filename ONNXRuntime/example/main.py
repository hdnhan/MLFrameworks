...
def infer(image: np.ndarray, session: ort.InferenceSession) -> np.ndarray:
    """Runs the input image through the network.

    Args:
        image (np.ndarray): The input image (1, 3, h, w).
        session (ort.InferenceSession): The ONNXRuntime session.
...
def warmup(session: ort.InferenceSession) -> None:
    ...
    output = infer(image, session)
    bboxes, scores, class_ids = postprocess(output, new_shape, (480, 640))


def main(ep: str, verbose: bool = False) -> None:
    # Load the network
    ...
    while cap.isOpened():
        ...
        # Inference
        start = perf_counter()
        detections = infer(image, session)
        end = perf_counter()
        t_infer = end - start
        t_infers += t_infer
